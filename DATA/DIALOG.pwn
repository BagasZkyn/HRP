...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_LOGIN)
    {
		...
		else
		{
			...
			else 
			{
				...
				ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Login to Home Roleplay", lstring, "Login", "Quit");
			}
		}
        return 1;
    }
	if(dialogid == DIALOG_REGISTER)
    {
		...
		if (strlen(inputtext) <= 5) return ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration Home Roleplay", "Kata sandi minimal 5 Karakter!\nMohon isi Password dibawah ini:", "Register", "Tolak");
		
		if(!IsValidPassword(inputtext))
		{
			Error(playerid, "Sandi valid : A-Z, a-z, 0-9, _, [ ], ( )");
			ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration Home Roleplay", "Kata sandi yang anda gunakan mengandung karakter yang valid!\nMohon isi Password dibawah ini:", "Register", "Tolak");
			return 1;
		}
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
                    ...
                    for(new i = 0; i < 5; i++)
				    {
					    PlayerTextDrawShow(playerid, HBESS[playerid][i]);
					    PlayerTextDrawShow(playerid, BARMAKAN[playerid]);
					    PlayerTextDrawShow(playerid, BARMINUM[playerid]);
					    PlayerTextDrawShow(playerid, TEXTMAKAN[playerid]);
					    PlayerTextDrawShow(playerid, TEXTMINUM[playerid]);
					    PlayerTextDrawShow(playerid, IDPLAYER[playerid]);
					    PlayerTextDrawShow(playerid, UANGPLAYER[playerid]);
				    }
				...
				case 1:
				{
					...
					for(new i = 0; i < 5; i++)
				    {
					    PlayerTextDrawHide(playerid, HBESS[playerid][i]);
					    PlayerTextDrawHide(playerid, BARMAKAN[playerid]);
					    PlayerTextDrawHide(playerid, BARMINUM[playerid]);
					    PlayerTextDrawHide(playerid, TEXTMAKAN[playerid]);
					    PlayerTextDrawHide(playerid, TEXTMINUM[playerid]);
					    PlayerTextDrawHide(playerid, IDPLAYER[playerid]);
					    PlayerTextDrawHide(playerid, UANGPLAYER[playerid]);
				    }
			...
			else if(bData[bizid][bType] == 3)
			{
				switch(listitem)
				{
					...
					case 1:
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TOYBUY, DIALOG_STYLE_LIST, ""RED_E"Home Roleplay RP: "WHITE_E"Player Toys", string, "Select", "Cancel");
					}
					case 2:
					{
						if(price == 0)
		...
		if(response) 
		{
			...
			foreach(new i : PVehicles)
			{
				if(carid == pvData[i][cVeh])
				{
					...
					format(pvData[i][cPlate], 32, "Home Roleplay-%d", rand);
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 2: // change bone
				{
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSISI, DIALOG_STYLE_LIST, ""RED_E"Home Roleplay RP: "WHITE_E"Player Toys", finstring, "Select", "Cancel");
				}
				case 3:
				{
	...
	if(dialogid == DIALOG_BANKDEPOSIT)
	{
		...
		else
		{
			...
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Home Roleplay RP: "LB_E"Bank", lstr, "Close", "");
		}
	}
	...
	if(dialogid == DIALOG_VERIFYCODE)
	{
		if(response)
		{
			...
			if(strval(inputtext) == pData[playerid][pVerifyCode])
			{
				...
				ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration Home Roleplay", lstring, "Register", "Quit");	
				return 1;
			}
	...
	if(dialogid == DIALOG_SERVERMONEY_STORAGE)
	{
		...
		else 
		{
			...
			ShowPlayerDialog(playerid, DIALOG_SERVERMONEY, DIALOG_STYLE_MSGBOX, "Home Roleplay City Money", lstr, "Manage", "Close");
		}
	}
	...
	if(dialogid == GARAGE_SPAWNFV)
	{
	    if(!response) return 1;
	    
	    new gID = GetPVarInt(playerid, "garageID");
        new gvehID = GetPVarArray(playerid, "GarageVehList", listitem);

        if (!Iter_Contains(GarageVehicles, gvehID)) return SendClientMessage(playerid, -1, "Vehicle no longer valid.");

        new Float:x = gVehData[gvehID][gvX];
        new Float:y = gVehData[gvehID][gvY];
        new Float:z = gVehData[gvehID][gvZ];
        new Float:a = gVehData[gvehID][gvAngle];
        new model = gVehData[gvehID][gvModel];

        new vehid = CreateVehicle(model, x, y, z, a, -1, -1, -1);
        gVehData[gvehID][gvVehID] = vehid;
        gVehData[gvehID][gvSpawned] = true;

        PutPlayerInVehicle(playerid, vehid, 0);
        SendClientMessage(playerid, -1, "Vehicle spawned.");
	}
	
	if(dialogid == GARAGE_EDIT)
	{
	    if(!response) return 1;
	    
	    new id = GetPVarInt(playerid, "garageEditID");
            if(!Iter_Contains(Garages, id)) return Error(playerid, "Garage no longer valid.");

            if(listitem == 0) return CallLocalFunction("cmd_editgaragepos", "si", playerid, id);
            if(listitem == 1)
                return ShowPlayerDialog(playerid, GARAGE_ADDVEHICLE, DIALOG_STYLE_INPUT,
                    "Add Vehicle to Garage", "Enter vehicle model ID:", "Add", "Cancel");
            if(listitem == 2)
            {
                ...
                SendClientMessage(playerid, -1, "Teleported to garage.");
            }
	}
	
	if(dialogid == GARAGE_ADDVEHICLE)
	{
	    if(!response) return 1;
	    
	    new id = GetPVarInt(playerid, "garageEditID");
            if(!Iter_Contains(Garages, id)) return Error(playerid, "Garage not valid.");

            new model = strval(inputtext);
            if(model < 400 || model > 611)
                return SendClientMessage(playerid, -1, "Invalid vehicle model ID.");

            new slot = Iter_Free(GarageVehicles);
            if(slot == -1) return Error(playerid, "Garage vehicle limit reached.");

            new Float:x = garageData[id][garagePosX], Float:y = garageData[id][garagePosY], Float:z = garageData[id][garagePosZ];

            gVehData[slot][gvGarageID] = id;
            gVehData[slot][gvModel] = model;
            gVehData[slot][gvX] = x;
            gVehData[slot][gvY] = y;
            gVehData[slot][gvZ] = z;
            gVehData[slot][gvAngle] = 0.0;
            gVehData[slot][gvSpawned] = false;
            gVehData[slot][gvVehID] = INVALID_VEHICLE_ID;

            Iter_Add(GarageVehicles, slot);

            new query[256];
            mysql_format(g_SQL, query, sizeof(query),
                "INSERT INTO garage_vehicles (garage_id, model_id, x, y, z, angle) VALUES (%d, %d, %f, %f, %f, %f)",
                id, model, x, y, z, 0.0);
            mysql_tquery(g_SQL, query);

            SendClientMessage(playerid, -1, "Garage vehicle added successfully.");
        }
	}

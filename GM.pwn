//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
		HOME ROLEPLAY PROJECT 2025
        gamemode version : HRP 0.1F
        scripter : Nathan
*/
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//PROJEK SSRP
//#include "DYNAMIC\factionveh.pwn"
//-----[ Discord Status ]-----	
forward BotStatus();
public BotStatus()
{
...
public OnGameModeInit()
{
	...
	mysql_tquery(g_SQL, "SELECT * FROM `garages`", "OnGaragesLoaded");
	mysql_tquery(g_SQL, "SELECT * FROM `garage_vehicles`", "OnGarageVehiclesLoaded");
...
public OnPlayerSpawn(playerid)
{
	...
	PlayerTextDrawSetString(playerid, IDPLAYER[playerid], string );
...
public OnPlayerEnterDynamicCP(playerid, checkpointid)
{
	...
	{
		ShowPlayerDialog(playerid, DIALOG_BUYPVCP, DIALOG_STYLE_LIST, "{7fffd4}Home Roleplay Showroom", "Motorcycle\nMobil\nKendaraan Unik\nKendaraan Job", "Select", "Cancel");
	}
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if (newkeys & KEY_SECONDARY_ATTACK)
    {
        ...
        {
            if (IsPlayerInRangeOfPoint(playerid, 4.0, garageData[i][garagePosX], garageData[i][garagePosY], garageData[i][garagePosZ]))
            {
                if (pFaction[playerid] != garageData[i][garageFactionID])
                    return SendClientMessage(playerid, -1, "You are not part of this garage's faction.");

                new list[1024], count, gvIndex[MAX_GARAGE_VEHICLES];
                for (new v = 0; v < MAX_GARAGE_VEHICLES; v++)
                {
                    if (!Iter_Contains(GarageVehicles, v)) continue;
                    if (gVehData[v][gvGarageID] != garageData[i][garageID]) continue;
                    if (gVehData[v][gvSpawned]) continue;

                    format(list, sizeof list, "%sModel: %d (GarageVeh ID: %d)\n", list, gVehData[v][gvModel], gVehData[v][gvID]);
                    gvIndex[count++] = v;
                }

                if (count == 0)
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "Garage", "No vehicles available.", "OK", "");

                SetPVarInt(playerid, "garageID", garageData[i][garageID]);
                SetPVarArray(playerid, "GarageVehList", gvIndex, count);
                ShowPlayerDialog(playerid, GARAGE_SPAWNFV, DIALOG_STYLE_LIST, "Select vehicle to spawn", list, "Spawn", "Cancel");
                return 1;
            }
        }
    }
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
    ...
    {	
		...
		for(new i = 0; i < 4; i++)
		{
		    PlayerTextDrawHide(playerid, HBEVEH[playerid][i]);
		    PlayerTextDrawHide(playerid, SPEEDVEH[playerid]);
		    PlayerTextDrawHide(playerid, NAMEVEH[playerid]);
		    PlayerTextDrawHide(playerid, HEALVEH[playerid]);
	...
	else if(newstate == PLAYER_STATE_DRIVER)
    {
		...
		if(pData[playerid][pHBEMode] == 1)
		{
			for(new i = 0; i < 4; i++)
			{
			    PlayerTextDrawShow(playerid, HBEVEH[playerid][i]);
			    PlayerTextDrawShow(playerid, SPEEDVEH[playerid]);
			    PlayerTextDrawShow(playerid, NAMEVEH[playerid]);
			    PlayerTextDrawShow(playerid, HEALVEH[playerid]);
...
UpdatePlayerHBE(playerid)
{
	new Float:hunger, Float:energy;
  	...
  	hunger = pData[playerid][pHunger] * 82.5/100;
	PlayerTextDrawTextSize(playerid, BARMAKAN[playerid], hunger, 11.0);
 	PlayerTextDrawShow(playerid, BARMAKAN[playerid]);

 	energy = pData[playerid][pEnergy] * 82.5/100;
	PlayerTextDrawTextSize(playerid, BARMINUM[playerid], energy, 11.0);
 	PlayerTextDrawShow(playerid, BARMINUM[playerid]);
	...
	if(IsValidVehicle(vehicleid))
	{
		...
		if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			if(pData[playerid][pHBEMode] == 1)
			{
				...
				new tstr[64], damagestr[62], veh[20];
				...
				format(damagestr, sizeof(damagestr), "%s%.2f", veh, fDamage / 10);
				PlayerTextDrawSetString(playerid, HEALVEH[playerid], damagestr);

				format(tstr, sizeof(tstr), "%i%", GetVehicleFuel(vehicleid));
				PlayerTextDrawSetString(playerid, FUELVEH[playerid], tstr);

				format(tstr, sizeof(tstr), "%i", GetVehicleSpeedKMH(vehicleid));
				PlayerTextDrawSetString(playerid, SPEEDVEH[playerid], tstr);

				format(tstr, sizeof(tstr), "%s", GetVehicleName(vehicleid));
				PlayerTextDrawSetString(playerid, NAMEVEH[playerid], tstr);
				/*
				if(!GetEngineStatus(vehicleid))
				{
...
ptask PlayerUpdate[999](playerid)
{
	...
	if(pData[playerid][pHBEMode] == 1 && pData[playerid][IsLoggedIn] == true)
	{
			...
			PlayerTextDrawSetString(playerid, TEXTMAKAN[playerid], string );
			format(string, sizeof(string), "%d%", pData[playerid][pEnergy]);
			PlayerTextDrawSetString(playerid, TEXTMINUM[playerid], string );
			format(string, sizeof(string), "~g~$%d", pData[playerid][pMoney]);
			PlayerTextDrawSetString(playerid, UANGPLAYER[playerid], string );
		/*
...
stock SendDiscordMessage(channel, message[]) {
	...
	switch(channel)
	{
		...
		case 5:
		{
			ChannelId = DCC_FindChannelById("1379523777125417080");
			DCC_SendChannelMessage(ChannelId, message);
			return 1;
		}
		case 6://Korup
